...
func mapper() *reflectx.Mapper {
	...
	if mpr == nil {
		mpr = reflectx.NewMapperFunc(DEFALT_TAG_NAME, NameMapper)
...
func (db *DB) MapperFunc(mf func(string) string) {
	db.Mapper = reflectx.NewMapperFunc(DEFALT_TAG_NAME, mf)
}
...
func (db *DB) SetTagName(name string) {
	db.Mapper = reflectx.NewMapperFunc(name, NameMapper)
}
...
func unmarshalFields(
	...
	for i, field := range fields {
		...
		if f.Kind() == reflect.Float32 || f.Kind() == reflect.Float64 {
			return errors.Errorf("Type <%s> on column index <%d> is not allowed to use. Use <string>", f.Kind(), field[0])
		}
